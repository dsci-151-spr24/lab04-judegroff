...
There are `r nrow(dennys)` rows and `r ncol(dennys)` columns. Each row represents the data for a single Denny's restaurant, and the variables are: `r names(dennys)`.

2.  What are the dimensions of the La Quinta's dataset?
    What does each row in the dataset represent?
    What are the variables?
    
There are `r nrow(laquinta)` rows and `r ncol(laquinta)` columns. Each row represents the data for a single La Quintas Inn, and the variables are: `r names(laquinta)`.
...
La Quinta hotels are mostly located in the Americas, but also have locations in other areas of the world. "Denny's" are located in the US.
...
```
There are no "Denny's" outside the US in our data.
...
laquinta <- laquinta %>%
  mutate(country = case_when(
    ...
    state == "ANT"           ~ "Colombia",
    state %in% c("AG", "PU", "VE", "NL", "CH", "QR") ~ "Mexico",
    state == "FM"            ~ "Honduras"
  ))
```
...
The state with the least "Denny's" locations is "DE" with 1, the state with the most is "CA" with 403.

The state with the least "La Quinta" Inns is "ME" with 1, and the state with the most is "TX" with 237.
...
```
The state with the most "Denny's" per thousand square miles is District of Columbia.
...
The state with the most "La Quinta" inns per thousan square miles is Rhode Island.
...
```
It holds up some what, but not super well.
...
```
It does seem to hold up better here, lots of overlapping points in different colors.
